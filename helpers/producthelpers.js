...
var instance = new Razorpay({
    key_id: 'rzp_test_7MkKUlEq70YTed',
    key_secret: 'i247X9QBT4EvZsKG9QVblh6r',});
...
placeorder:(orderdetails,user,cart)=>{
    ...
    generatorRazorpay:(orderid,totalPrice)=>{
        return new Promise((resolve,reject)=>{
            var options = {
    ...
    verifyPayment:(paymentdetails)=>{
        return new Promise((resolve,reject)=>{
            console.log("veriyying..........")
            let hmac=crypto.createHmac('sha256','i247X9QBT4EvZsKG9QVblh6r');
            hmac.update(paymentdetails['payment[razorpay_order_id]'] + "|" + paymentdetails['payment[razorpay_payment_id]']);
            hmac=hmac.digest('hex')
            console.log(hmac)
            let razorpay_signature=paymentdetails['payment[razorpay_signature]']
    ...
    changeStatus:(orderid)=>{
        console.log("cartstatus update")
        console.log(orderid)
        return new Promise(async (resolve,reject)=>{
            await db.get().collection(collection.ordercollection).updateOne({_id:objectid(orderid)},{$set:{status:"Placed Successfully"}})
            resolve(orderid)
        })
    }
